...
class bert_model(object):
	def __init__(self):
		...
		model = BertForSequenceClassification.from_pretrained(args.bert_model, state_dict=model_state_dict, num_labels=self.num_labels)
		model.to(self.device)
		self.model = model

	def predict_label(self, turn, personas_items, persona_triples):
		eval_examples = self.processor.create_batch(turn, personas_items)
		eval_features = convert_examples_to_features(eval_examples, self.label_list, self.args.max_seq_length, self.tokenizer)
		...
		for input_ids, input_mask, segment_ids, label_ids in tqdm(eval_dataloader, desc="Evaluating"):
			...
			for idx_p, val_p in enumerate(idx_max):
				if(mapper[val_p]=="entailment"):
					p_res.append(persona_triples[idx_p])
					# value.append(val_max[idx_p])
				# else:
				# 	p_res.append("None")
				# 	value.append(0.0)
